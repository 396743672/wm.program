...
public class JWThelper {
	 ...
	 * @see
	 */
	public static IJWTinfo getInfoFromToken(String token, byte[] pubKeyByte)
			throws ExpiredJwtException, UnsupportedJwtException, MalformedJwtException, SignatureException,
	 ...
	 * @see
	 */
	private static Jws<Claims> parserToken(String token, byte[] pubKeyByte)
			throws ExpiredJwtException, UnsupportedJwtException, MalformedJwtException, SignatureException,
			IllegalArgumentException, InvalidKeySpecException, NoSuchAlgorithmException {
		Jws<Claims> claimsJws = Jwts.parser().setSigningKey(RsaKeyHelper.getInstance().getPublicKey(pubKeyByte))
				.parseClaimsJws(token);
		return claimsJws;
	}
	 ...
	 * @see
	 */  
	public static String generateToken(IJWTinfo jwtInfo, byte[] userPriKey, Integer expire) throws NoSuchAlgorithmException, InvalidKeySpecException {
		String compactJws = Jwts.builder().setSubject(jwtInfo.getUniqueName())
				.claim(CommonConstants.JWT_KEY_ID, jwtInfo.getId())
				.claim(CommonConstants.JWT_KEY_NAME, jwtInfo.getName())
				.setExpiration(DateTime.now().offset(DateField.MINUTE, expire))
				.signWith(SignatureAlgorithm.RS256, RsaKeyHelper.getInstance().getPrivateKey(userPriKey)).compact();
		return compactJws;
	}
}
